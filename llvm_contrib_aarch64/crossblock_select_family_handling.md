...
```
  ┌──────────────────────────────────────────────────────────────────┐
  │                    MachineFunction (all blocks)                  │
  │          Visited in Dominator Tree pre-order via run()           │
  └────────────────────────┬────────────────────────────────────────┘
                           │  for each block HBB
             ┌─────────────┴──────────────┐
             ▼                            ▼
    optimizeIntraBlock()        optimizeCrossBlock()
    ┌──────────────────┐        ┌──────────────────────────────────┐
    │ Scans a single   │        │ Looks at HBB + its true-successor│
    │ block forward    │        │ TBB in the dominator tree        │
    │                  │        └──────────────┬───────────────────┘
    │ Finds CMP+Select │                       │
    │ pairs within the │          ┌────────────┴────────────┐
    │ same block and   │          ▼                         ▼
    │ adjusts them so  │   Find consumer in HBB      Find consumer in TBB
    │ CMPs match for   │   ┌─────────────────┐        ┌─────────────────┐
    │ CSE elimination  │   │ getBccTerminator│        │ getBccTerminator │
    └──────────────────┘   │   (Bcc path)   │        │   (Bcc path)    │
                           │       OR        │        │       OR         │
                           │findSelectConsum │        │findSelectConsum  │
                           │ (CSEL/CSET/etc) │        │ (CSEL/CSET/etc) │
                           └────────┬────────┘        └────────┬────────┘
                                    │                          │
                           ┌────────┴──────────────────────────┘
                           │
                ...
                ┌─────────────────────┐
                │   Safety Checks     │
                │ • nzcvLivesOut(HBB) │  ← must be false
                │ • nzcvLivesOut(TBB) │  ← must be false
                │ • registersMatch()  │  ← both CMPs compare same reg
                │ • findAdjustableCmp │  ← CMP must be adjustable
                └──────────┬──────────┘
                           │
                           ▼
                ┌─────────────────────┐
                │  tryOptimizePair()  │
                │                     │
                │  Case 1: Opposite   │   GT(#9) + LT(#11)  → GE(#10) + LE(#10)
                │  (diff == 2)        │   CMPs now identical → CSE removes one
                │                     │
                │  Case 2: Same-dir   │   GT(#9) + GT(#10)  → GE(#10) + GT(#10)
                │  (diff == 1)        │   CMPs now identical → CSE removes one
                └──────────┬──────────┘
                           │
                           ▼
                ┌─────────────────────┐
                │  applyCmpAdjustment │
                │  updateCmpInstr()   │  ← adjusts immediate + opcode
                │  updateCondInstr()  │  ← adjusts condition code
                └─────────────────────┘
