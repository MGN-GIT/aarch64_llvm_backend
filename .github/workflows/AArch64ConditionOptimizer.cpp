...
//
// The pass handles:
//  * Cross-block: SUBS/ADDS followed by conditional select and branches
...
AArch64ConditionOptimizerImpl::findSelectConsumer(MachineBasicBlock *MBB) {
  ...
  for (MachineInstr &MI : reverse(*MBB)) {
    ...
    if (!Found) {
      ...
      // A NZCV writer below the found select means the select is not reading
      // from the CMP we will find further back. Stop.
      if (MI.modifiesRegister(AArch64::NZCV, /*TRI=*/nullptr))
        return {nullptr, AArch64CC::Invalid};
    }
  }
